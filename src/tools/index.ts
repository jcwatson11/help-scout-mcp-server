...
export class ToolHandler {
  ...
  /**
   * Write-operation tool definitions. Deliberately NOT part of listTools()
   * (which feeds the read-only gateway registry). These are advertised and
   * dispatched through the separate write_help_scout gateway, gated by
   * writes.enabled. Handlers live in callTool()'s switch below.
   */
  listWriteTools(): Tool[] {
    return [
        {
    ...
    try {
      ...
      switch (request.params.name) {
        ...
        case 'getInbox':
          ...
          break;
        // Write operations — reachable by name (e.g. via the write_help_scout
        // gateway); gated inside each handler by assertWritesEnabled().
