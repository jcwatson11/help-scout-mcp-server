...
describe('ToolHandler', () => {
  ...
  describe('listTools', () => {
    it('should return all available read operations', async () => {
      process.env.HELPSCOUT_ENABLE_WRITES = 'true';
      const tools = await toolHandler.listTools();

      // Write operations are NOT in listTools(); they are exposed via the
      // separate write_help_scout gateway. listTools() is read-only (55 ops).
    ...
    it('never exposes write operations through listTools() (read-only surface)', async () => {
      // listTools() feeds the read-only gateway registry, so it must never
      // contain write operations regardless of HELPSCOUT_ENABLE_WRITES.
      // Writes are advertised separately via the write_help_scout gateway.
      process.env.HELPSCOUT_ENABLE_WRITES = 'true';
      const tools = await new ToolHandler().listTools();
